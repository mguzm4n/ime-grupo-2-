...
# Añadimos la variable dependiente y la que se pide próximamente en el punto (4).
...
# Graficar el modelo obtenido anteriormente para ver la recta:
...
# Comenzamos con el modelo más simple (el construido anteriormente) 
# para llegar al modelo completo (con las ocho variables aleatorias seleccionadas)
...
# un modelo medianamente aceptable.
...
# la opción más simple de validación cruzada, sin utilizar la función trainControl (por eso se comenta).
...
# es bastante notoria, y por ello se concluye, realizado todo el trabajo
# anterior, que el error es bastante grande, corroborando que
